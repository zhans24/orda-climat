...
class RootCategoriesTable extends BaseWidget
{
    protected static ?string $heading = 'Родительские категории';
        ...
        return $table
            ...
            ->headerActions([
                Action::make('createRoot')
                    ->label('Создать родительскую')
                    ->icon('heroicon-m-plus')
                    ->url(fn () => CategoryResource::getUrl('create', ['type' => 'root']))
                    ->color('primary'),
            ])
            ->recordActions([
                EditAction::make()
